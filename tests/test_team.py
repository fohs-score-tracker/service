...
def create_team(client: TestClient) -> Response:
    ...
    client.post(
        "/users/new",
        json={"name": "test", "password": "test", "email": "jeff@example.com"},
    )
    return client.post(
        "/teams/new", json={"name": "testTeam", "players": [1], "coaches": [1]}
    )
...
def test_new_team_201(client: TestClient):
    ...
    assert response.status_code == 201 and response.json() == {
        "id": 1,
        "name": "testTeam",
        "coaches": [{"name": "test", "email": "jeff@example.com", "id": 1}],
        "players": [{"id": 1, "name": "TestJeff", "shots": []}],
    }
...
def test_team_edit_200(client: TestClient):
    ...
    response = client.patch(
        "/teams/1", json={"name": "Home Team", "players": [1], "coaches": [1]}
    )
    assert response.status_code == 200 and response.json() == {
        "id": 1,
        "name": "Home Team",
